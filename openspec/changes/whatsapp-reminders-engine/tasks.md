...
- [ ] 2.2 Adicionar tabela `whatsapp_messages` em `src/shared/db/schema/whatsapp/tables.ts` — colunas: id (uuid PK), user_id (uuid NOT NULL, FK auth.users), patient_id (uuid nullable, FK patients), session_id (uuid nullable, FK sessions), direction (text NOT NULL, CHECK 'outbound'/'inbound'), to_phone (varchar 20), from_phone (varchar 20), body (text), template_key (varchar 50), bsp_message_id (varchar 255), idempotency_key (varchar 64), status (text, CHECK 'queued'/'sent'/'delivered'/'read'/'failed'/'unable_to_send'), error_reason (text), sent_at (timestamptz), delivered_at (timestamptz), read_at (timestamptz), created_at (timestamptz DEFAULT now()). Indexes: (user_id, created_at DESC), (session_id), (patient_id, created_at DESC). UNIQUE parcial em bsp_message_id (WHERE bsp_message_id IS NOT NULL). UNIQUE parcial em idempotency_key (WHERE status != 'failed' AND idempotency_key IS NOT NULL)
- [ ] 2.3 Adicionar coluna `reminders_disabled` (boolean DEFAULT false) ao schema Drizzle de `sessions` em `src/shared/db/schema/agenda/tables.ts`
- [ ] 2.4 Criar/estender `src/shared/db/schema/whatsapp/policies.ts` — RLS policies: reminder_settings usa `user_id = auth.uid()` (SELECT/INSERT/UPDATE/DELETE). whatsapp_messages usa `user_id = auth.uid()` (SELECT/INSERT/UPDATE). Seguir pattern de `src/shared/db/schema/patients/policies.ts`
- [ ] 2.5 Atualizar `src/shared/db/schema/whatsapp/index.ts` — barrel reexportando tables e policies (incluindo tabelas da change 1)
- [ ] 2.6 Atualizar `src/shared/db/schema/index.ts` — garantir reexport de `./whatsapp` (se change 1 ja fez, apenas verificar)
- [ ] 2.7 Rodar `npm run db:generate`, editar migration para incluir: RLS policies SQL, FK constraints manuais, CHECK constraints para direction/status enums, ALTER TABLE sessions ADD COLUMN reminders_disabled BOOLEAN DEFAULT false
- [ ] 2.8 Testar migration com `npm run db:migrate` local
- [ ] 2.9 **Teste de integracao:** Criar `src/__tests__/integration/whatsapp/reminders-schema.int.test.ts` — verificar: tabelas reminder_settings/whatsapp_messages existem, RLS habilitado, CHECK constraints funcionam (direction invalido rejeitado, status invalido rejeitado), indexes existem, FK constraints funcionam, UNIQUE parcial em bsp_message_id funciona, UNIQUE parcial em idempotency_key funciona, coluna reminders_disabled existe em sessions com default false

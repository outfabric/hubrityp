...
This delta re-aligns the homepage sections to their canonical Figma frames in the
"Hubrity Design System" file (key `HoLOEqq9PXlo6IwLkz3FQ9`) at desktop 1440 and
mobile 375. Token names below are the DS variables read from Figma via
`get_variable_defs`; structure/copy/dimensions are read via `get_metadata`.

DS token legend used across these requirements (exact values from Figma):
`brand-50 #f2f5f1`, `brand-100 #e1e8de`, `brand-200 #c2d1bc`, `brand-400 #7e9e78`,
`brand-600 #587355`, `brand-700 #475d45`; `text-primary #1c1917`,
`text-secondary #57534e`, `text-tertiary #78716c`, `text-inverse #fafaf9`;
`background #fafaf9`, `surface #ffffff`, `surface-muted #f5f5f4`,
`surface-sunken #f0efec`; `border #e7e5e4`, `border-subtle #efedeb`,
`border-strong #d6d3d1`. Type tokens: `Display/xl` 52/56 (-0.5), `Display/lg`
40/46 (-0.4), `Display/md` 32/40 (-0.2), `Heading/h2` 22/28, `Heading/h3` 18/24,
`Heading/h4` 16/22, `Lead` 20/30, `Body/lg` 17/28, `Body/base` 15/22, `Body/sm`
13/20, `Label/caption` 12/16 (ls 0), `Label/caption-upper` 12/16 (ls 6) — all
Inter, weights ≤ 600. Spacing scale (px): 4/8/12/16/20/24/32/40/48/64/96.
Radius: `lg 10`, `xl 12`, `2xl 16`, `full 9999`.

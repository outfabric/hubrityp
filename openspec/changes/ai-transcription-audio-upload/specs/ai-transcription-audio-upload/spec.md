...
### Requirement: Recording consent gating in telepsicologia

The system SHALL gate the start of a video-session recording on TWO independent checks: the legacy `patients.recording_consent_signed_at` field (preserved during the MVP transition) AND `assertAiConsentActive` (new authority). If either is negative, recording does NOT start. The recording-cleanup pipeline emits `ai-transcription/recording.completed` upon Stream confirmation, which `ingestStreamRecording` consumes (in the `ai-transcription-audio-upload` capability) to bring the audio into our bucket.

#### Scenario: Both checks pass
- **WHEN** both legacy and new gates are positive
- **THEN** recording starts and Stream begins capturing

#### Scenario: Only legacy passes
- **WHEN** legacy is set but no `ai_recording` term exists
- **THEN** recording does NOT start

#### Scenario: Only new passes (legacy absent — fresh patient)
- **WHEN** the patient has the new AI term signed but no legacy field migrated
- **THEN** recording does NOT start until the legacy field is also set (or removed in a future cleanup change)
- **AND** the UI guides the psychologist (out of scope of this change; documented in the run-book)

...
- [ ] 6.2 Create `src/modules/telepsicologia/server/extend-session.ts` — Server Action: (1) authenticate, (2) validate input (room_id), (3) verify room ownership and status='active', (4) UPDATE video_rooms SET expires_at = expires_at + INTERVAL '15 minutes', (5) INSERT video_session_logs event_type='session_extended', (6) return { ok: true }
...
- [ ] 6.4 **Integration test:** Create `src/__tests__/integration/telepsicologia/extend-session.int.test.ts` — tests: happy path extends expires_at by 15 min + logs event, room not active -> rejected, room not owned -> rejected

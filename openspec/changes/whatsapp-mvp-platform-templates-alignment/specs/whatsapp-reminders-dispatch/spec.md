# whatsapp-reminders-dispatch — Delta
...
**Reason**: The 12-variable PT dictionary no longer drives outbound sends; the real platform Content templates declare named variables (`first_name`, `professional_name`, `date`, `time`, `session_link`) and Twilio substitutes them server-side.
**Migration**: Use `buildContentVariables(templateKey, ctx)` from the platform template contract (see ADDED requirement). `select-template-variables.ts` is deleted; the dictionary in `template-variables.ts` remains as UI metadata only.

### Requirement: Video link template variable is populated from video_rooms

**Reason**: Local rendering is gone; the video URL is resolved by the dispatcher and passed as the `session_link` named variable, and an unresolvable link now skips the send instead of substituting an empty string.
**Migration**: See ADDED requirement "Video link reminder is skipped when the session link is unavailable".

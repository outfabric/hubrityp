# subscription-plans-config Specification

## Purpose

Defines the central, Zod-validated subscription plans configuration that is the single source of truth for both the homepage pricing summary and the `/precos` page, the MVP-only feature communication guard, and the empty-plans safety fallback. Created by syncing change `public-site-foundation`.

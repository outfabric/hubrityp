# ai-transcription-env-config Specification

## Purpose

Defines the environment variables, Zod validation schemas, and `.env.example` entries required to configure the AI transcription feature (Gemini API keys, audio storage settings). Created by syncing change `ai-transcription-foundation`.

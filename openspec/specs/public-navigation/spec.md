...
The system SHALL render a footer on every public page (rendered in the dark surface), containing: the Hubrity logo and tagline; a "Produto" column (Funcionalidades anchor, Preços); a "Legal" column (Política de Privacidade, Termos de Uso, LGPD → privacy-page LGPD anchor); a "Contato" column (support email `suporte@hubrity.com.br`); and a copyright line "© 2026 Hubrity. Dados armazenados no Brasil. Feito para psicólogos autônomos brasileiros." The footer component MUST be reusable by the authenticated app.

#### Scenario: Footer legal links resolve to functional pages

- **WHEN** the footer renders
- **THEN** "Política de Privacidade" links to `/politica-de-privacidade`, "Termos de Uso" links to `/termos-de-uso`, "LGPD" links to the LGPD section anchor of the privacy page, and the support email is a `mailto:` link

# public-email-confirmation Specification

## Purpose

Define the public, session-less email-confirmation surface: the `/verifique-email` page reached by a just-signed-up (and therefore anonymous) psychologist, the signed `pending-email` cookie that carries the target address server-side instead of in the URL, the masked-email rendering, the enumeration-safe anonymous resend action, and the shared confirm-email copy reused on the login page's `email_not_confirmed` state. Created by archiving change `add-public-email-confirmation`.

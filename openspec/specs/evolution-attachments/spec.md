# evolution-attachments Specification

## Purpose

Defines the file attachment capability within the electronic medical record: upload of PDF, JPG, PNG, MP3, and MP4 files (up to 50MB) with server-side magic-bytes MIME validation, UUID-based Storage paths under `${user_id}/${patient_id}/`, category classification, audio consent enforcement (CFP 13/2022), signed URL preview (5-minute expiry), soft-delete with 20-year retention mandate (no physical deletion), Supabase Storage per-user prefix isolation policies, RLS user_id-scoped isolation on `evolution_attachments` (no DELETE policy), and audit logging on upload, view-url generation, and delete operations. Created by archiving change `prontuario-attachments-and-personal-notes`.

...
The system SHALL render a patient detail page at `/app/pacientes/:id` with a header showing: photo (or initials), full_name, calculated age (from birth_date), phone (with "Abrir no WhatsApp" button), email (with "Copiar" button), tags as chips, and status badge.

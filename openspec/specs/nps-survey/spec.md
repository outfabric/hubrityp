# nps-survey Specification

## Purpose
Day-7 NPS survey for the authenticated psychologist: a one-time modal whose eligibility is derived server-side from `first_access_at` and `nps_responded_at`, an owner-scoped validated `submitNps` Server Action (with dismissal support and a later-submission path from Configurações > Feedback), a detractor follow-up email free of PII in logs, and service-role-only Inngest scheduling that is not reachable from a public request path. Created by syncing the `in-app-notifications-and-nps` change.

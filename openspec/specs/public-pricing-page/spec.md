# public-pricing-page Specification

## Purpose

Defines the public pricing page at `/precos` (inside the `(public)` layout): its structure (title/subtitle, 2 plan cards, expandable comparison table, billing FAQ, final CTA), the central-config-driven plan cards (Essencial/Avançado), the Essencial⊂Avançado comparison-table invariant, the billing FAQ topics, and the MVP-only + empty-plans safety constraints. Created by syncing change `public-pricing-page`.

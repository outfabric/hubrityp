...
## Purpose
In-app notifications for the authenticated psychologist: a header notification bell with an owner-scoped unread count, a dropdown listing MVP notification types with per-type icons and pt-BR relative timestamps, IDOR-safe session-scoped read actions, realtime unread-count updates, a 30-day auto-read scheduled job, and an owner-scoped notification preferences page with critical email locked on. Created by syncing the `in-app-notifications-and-nps` change.

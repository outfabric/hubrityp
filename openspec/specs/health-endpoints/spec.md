...
## Purpose
Define the platform's diagnostic HTTP surface: the public `/api/health` Route
Handler that probes the database via Drizzle and the authenticated `/api/me`
Route Handler that reports the active session's identity. Both endpoints
exist to give operators and integration tests a contract-level pulse on the
system. Created by archiving change `smoke-health-feature`.

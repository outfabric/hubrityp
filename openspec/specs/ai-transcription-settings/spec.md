# ai-transcription-settings Specification

## Purpose
Settings surface for the AI transcription feature: a dedicated `/configuracoes/transcricao-ia` page where the psychologist enables/disables the feature, picks the default note template, risk-detection sensitivity and retention, and views aggregated usage statistics. Persistence is audited (no PII) and RLS-scoped.

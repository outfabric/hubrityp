# psychometric-scales Specification

## Purpose

Defines the psychometric scales capability within the electronic medical record: a built-in library of public-domain scales (PHQ-9, GAD-7, SDQ, AUDIT, WHOQOL-Bref) with validated scoring and classification, in-session application by the psychologist, secure token-gated remote patient self-report via `/escala/[token]`, rate-limited public endpoints, Inngest cron for token expiry observability, longitudinal Recharts visualization of score evolution, Escalas tab integration in the prontuario shell, RLS user_id-scoped isolation (no DELETE policy per Lei 13.787/2018), audit logging on all operations, LGPD notice on the public page, and cryptographically secure token generation (256-bit entropy). Created by archiving change `prontuario-scales-application`.

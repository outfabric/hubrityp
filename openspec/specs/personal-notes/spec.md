...
Defines the personal notes capability within the electronic medical record: a private rich-text note (Tiptap) per patient with auto-save, regulatory banner (Resolucao CFP 001/2009, art. 5), optional argon2id-hashed extra password with brute-force lockout state machine (5 attempts / 15-minute lock), per-patient isolation, explicit exclusion from default prontuario export, RLS user_id-scoped isolation (no DELETE policy per retention mandate), and audit logging on every read, write, password, and lockout event. Created by archiving change `prontuario-attachments-and-personal-notes`.

# medical-records-foundation Specification

## Purpose

Defines the foundational schema, module structure, and shell UI for the electronic medical records (prontuario) domain. Establishes the Drizzle tables (`evolutions`, `evolution_versions`, `audit_log`) with RLS enabled and no DELETE policies (Lei 13.787/2018 mandates 20-year retention), the canonical module barrel at `@/modules/medical-records`, the prontuario shell page with tabbed navigation, and the schema re-export for cross-domain joins. Created by archiving change `prontuario-foundation-and-evolutions`.

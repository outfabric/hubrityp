...
Defines how a HubrityP contributor boots a complete local Supabase stack via the official CLI, runs the Next.js app inside a docker-compose container that bridges into that stack, and finds documentation for the hybrid model (CLI for dev, Testcontainers for tests). Created by archiving change `bootstrap-data-and-tests`.

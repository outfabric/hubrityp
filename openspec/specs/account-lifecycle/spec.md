...
## Purpose
Define the psychologist account lifecycle on the platform: the
`psychologist_profiles` table that stores profile and status, the typed
`transitionStatus` state machine that is the sole writer of `status`, the LGPD
consent timestamps captured at signup, the bloqueante pages
(`/auth/verify-email`, `/auth/crp-review`) shown for non-`active` accounts, the
`/auth/callback` Route Handler that advances status when the verification email
is consumed, and the `getAccountStatus` middleware helper that drives access to
authenticated areas. Created by archiving change `add-account-signup-and-lifecycle`.

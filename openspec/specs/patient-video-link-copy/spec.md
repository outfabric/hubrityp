# patient-video-link-copy Specification

## Purpose

Lets the psychologist quickly copy a patient's video join link from the agenda: a "Link do paciente" section in the session detail drawer, a post-scheduling toast action, and the supporting data plumbing (`SessionWithDetails.patientVideoUrl`, `MutationResult.patientVideoUrl`) that surfaces the patient video URL constructed server-side from the reserved room's `patient_token`.

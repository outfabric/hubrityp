...
afterEach(() => {
  // Clear any per-test `vi.doMock('@/lib/db', ...)` so the next test loads
  // the real Drizzle client. Without `doUnmock`, the mock factory persists
  // across tests because Vitest registers it globally for the test file.
  vi.doUnmock('@/lib/db');
  vi.resetModules();
});

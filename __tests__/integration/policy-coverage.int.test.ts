...
// Lint test: every table declared under db/schema/**/tables.ts must have at
// least one matching `CREATE POLICY ... ON <table>` line in
// db/migrations/**.sql. Forgetting RLS on a new table is a security bug; this
// test makes the omission a CI failure rather than a code-review near-miss.
//
// See db/migrations/README.md for the canonical owner-scoped policy template.
const ROOT = path.resolve(__dirname, '../..');

async function findTableNamesInSchema(): Promise<string[]> {
  const files = await fg('db/schema/**/tables.ts', { cwd: ROOT, absolute: true });
...
async function findTablesWithPolicies(): Promise<string[]> {
  const files = await fg('db/migrations/**/*.sql', { cwd: ROOT, absolute: true });
...
describe('RLS policy coverage', () => {
  it('every Drizzle table has at least one CREATE POLICY in db/migrations', async () => {

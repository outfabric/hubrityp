...
- [Public homepage QA](project_public_homepage_qa.md) — `/` 10-section structure + a11y handles, carousel/lightbox/FAQ/reduced-motion behaviors, DS no-gradient/blur/700 rules, and the long-CTA-label whitespace-nowrap mobile-overflow trap (sweep 320/375px)
...
- [Patient PhoneInput + lifecycle QA](project_patient_phone_input_and_lifecycle_qa.md) — +55 aria-hidden adornment + national-only editable mask, DDD-55 round-trip, archive revalidatePath cache fix; 2 MÉDIO traps: paste-mangling (no 55-strip in maskNationalPhone) + unarchive-shows-archive-modal-copy

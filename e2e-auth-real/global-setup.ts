...
// under `e2e-auth-real/.auth/`. We deliberately use a file (not env vars)
// because Playwright workers run in separate processes and any env state
// set here would not survive the boundary.
//
...
//
// `playwright.auth-real.config.ts` validates that the Supabase stack is
// running and exports the discovered URL + service-role key on
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    throw new Error(
      '[auth-real/global-setup] Missing AUTH_REAL_SUPABASE_URL or ' +
        ...
        'for populating these — did you bypass `playwright.auth-real.config.ts`?',
    );
  }

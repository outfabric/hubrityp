...
export default async function VideoCallPage({ params }: VideoCallPageProps) {
  ...
  // 5. Load video room for this session
  const [room] = await db
    .select()
    .from(videoRooms)
  ...
  // 6. No room: show "create room" UI
  if (!room) {
    return <CreateRoomCard sessionId={sessionId} patientName={patient?.fullName ?? null} />;
  }

  // 7. Room exists: mint a psychologist token to join the call
  //    Import the server action lazily to keep the import graph clean.
  ...
  // 8. Pass everything to the client component
  return (
    <VideoCallLoader

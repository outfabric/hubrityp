...
describe('markSessionNoShowImpl — Inngest event emission', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  // -----------------------------------------------------------------------
  // (a) Successful mark-no-show emits with noShowAt
  // -----------------------------------------------------------------------

  it('emits agenda/session.no_show with a noShowAt timestamp', async () => {
    const supabase = createMockSupabase(VALID_USER_ID);

    const result = await markSessionNoShowImpl(supabase, SESSION_ID);
    ...
    expect(sendArg.name).toBe('agenda/session.no_show');
    ...
    expect(sendArg.data.noShowAt).toBeInstanceOf(Date);
  ...
  it('logs structured error when inngest.send() fails', async () => {
    ...
    await markSessionNoShowImpl(supabase, SESSION_ID);
    ...
    expect(logPayload.eventName).toBe('agenda/session.no_show');
    expect(logPayload.sessionId).toBe(SESSION_ID);
    expect(logPayload.error).toBe('Connection refused');
    expect(logMessage).toBe('failed to send agenda/session.no_show event');
  });

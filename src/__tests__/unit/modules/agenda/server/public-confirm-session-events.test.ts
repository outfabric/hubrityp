...
const mocks = vi.hoisted(() => {
  ...
  // Public actions look up via .where().limit()
...
describe('publicConfirmSessionImpl — Inngest event emission', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  // -----------------------------------------------------------------------
  // (a) Successful confirm emits with confirmedBy: 'patient'
  // -----------------------------------------------------------------------

  it('calls inngest.send() with agenda/session.confirmed and confirmedBy: patient', async () => {
    ...
    expect(sendArg.data.confirmedBy).toBe('patient');
  ...
  // -----------------------------------------------------------------------
  // (c) Expired token does not emit an event
  // -----------------------------------------------------------------------

  it('does not emit an event when the confirmation token is expired', async () => {
    // Session has already started -> token is expired.
    mocks.setExistingSession(buildExistingSession({ startAt: pastDate() }));

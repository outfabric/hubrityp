...
function buildExistingSession(overrides: Record<string, unknown> = {}) {
  return {
    ...
    status: 'confirmed',
    // Recent updatedAt -> not affected by the 7-day lock.
    updatedAt: new Date(),
    ...overrides,
  };
}
...
describe('markSessionDoneImpl — Inngest event emission', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  // -----------------------------------------------------------------------
  // (a) Successful mark-done emits with doneAt
  // -----------------------------------------------------------------------

  it('emits agenda/session.done with a doneAt timestamp', async () => {
    const supabase = createMockSupabase(VALID_USER_ID);

    const result = await markSessionDoneImpl(supabase, SESSION_ID);
    ...
    expect(sendArg.name).toBe('agenda/session.done');
    ...
    expect(sendArg.data.doneAt).toBeInstanceOf(Date);
  ...
  it('logs structured error when inngest.send() fails', async () => {
    ...
    await markSessionDoneImpl(supabase, SESSION_ID);
    ...
    expect(logPayload.eventName).toBe('agenda/session.done');
    expect(logPayload.sessionId).toBe(SESSION_ID);
    expect(logPayload.error).toBe('Connection refused');
    expect(logMessage).toBe('failed to send agenda/session.done event');
  });

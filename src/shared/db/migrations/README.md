...
| `0042_premium_typhoid_mary.sql` | `telepsicologia` | Waiting-room arrival signal: adds `video_rooms.patient_waiting_at` (immutable first-arrival marker) and `video_rooms.patient_last_seen_at` (mutable liveness heartbeat), both nullable timestamptz, no new index (read by PK / watched in-row). Swaps `video_session_logs_event_type_check` (drop + recreate) to admit `patient_arrived`, the waiting-room arrival audit event, distinct from admission `patient_joined`. No new table, so no new RLS policies.                                   |

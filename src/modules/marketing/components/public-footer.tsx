...
/**
 * PublicFooter — the marketing site footer (task 7.1).
 *
 ...
 *   The brand mark uses the `tone="white"` Logo variant for legibility on dark.
 *
 * Accessibility:
...
/** Static copyright year — fixed per the design handoff (not render-time). */
...
const LEGAL_LINKS: ReadonlyArray<FooterLink> = [
  ...
  // Deep-links into the LGPD section anchor of the privacy page.
  { href: '/politica-de-privacidade#lgpd', label: 'LGPD' },
];
      ...
      <Container className="py-12">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-4">
          ...
          <div className="flex flex-col gap-4">
            <Logo variant="lockup-h" tone="white" className="h-8 w-auto" />
            <p className="text-text-secondary max-w-xs text-sm">
              Plataforma para psicólogos autônomos brasileiros
            </p>
          </div>
          ...
          <nav aria-labelledby={contatoHeadingId} className="flex flex-col gap-3">
            <h2 id={contatoHeadingId} className="text-text-primary text-sm font-semibold">
              Contato
            </h2>
            <ul className="flex flex-col gap-2">
              <li>
        ...
        <div className="border-border-subtle mt-10 border-t pt-6">
          <p className="text-text-tertiary text-sm">
            © {COPYRIGHT_YEAR} Hubrity. Dados armazenados no Brasil. Feito para psicólogos autônomos
            brasileiros.
          </p>
        </div>
      </Container>
